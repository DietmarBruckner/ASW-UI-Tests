    ...
    public class IDE_Main {
        ...
        static void InitUIElements() {
            foreach (AutomationElement a in MainWindow.FindAllChildren(_cf.ByControlType(ControlType.Pane))) {
                ...
                if (name.IndexOf("View", StringComparison.OrdinalIgnoreCase) >= 0)      ProjectExplorer = a;
                if (name.IndexOf("Output", StringComparison.OrdinalIgnoreCase) >= 0)    OutputWindow = a;
                if (autoId == "59648")                                                  Workspace = a;
                if (autoId == "6154")                                                   Toolbox = a;
                if (autoId == "6155")                                                   PropertyWindow = a;
